...
def returnSimilarPub(list_of_pubs, check_pub_title, check_pub_year, check_year):
    for pub in list_of_pubs:
        if(pub.title.lower() == check_pub_title.lower()):
          ...
          return pub
    for pub in list_of_pubs:
        if(check_pub_title=="Mechanisms and algorithms for aggressive replication systems"):
          print pub.title, check_word_containment(pub.title, check_pub_title)
...
# categories_publications(entries)
generate_publications_content('publications_content.html')

print years_entries[0][0].title
print years_entries[0][0].conference_name
print years_entries[0][0].entry
print conference_type(years_entries[0][0].entry)
print check_word_containment("Augmenting mobile 3G using WiFi", "Augmenting Mobile 3G Using WiFi: Measurement, Design, and Implementation")